...
- **双语界面**：设置中一键切换 跟随系统 / 中文 / English，全部文案与日期格式实时切换
...
>
> 首次构建时脚本会自动在你的登录钥匙串中创建一个名为 `MyClaude Dev` 的**自签名代码签名证书**，之后每次构建都用它签名。这样 App 的签名身份保持固定，钥匙串授权（如 GitHub 凭证）只需确认一次——若用默认的 ad-hoc 签名，每次重新编译都会被钥匙串当成陌生 App 反复弹窗。该证书仅存在于你自己的电脑上，可随时在「钥匙串访问」中删除。
...
**弹出「MyClaude 想要使用你存储在钥匙串的机密信息」？**
这是 macOS 在保护你连接 GitHub 时存入钥匙串的凭证：当 App 的签名身份发生变化（首次运行、版本更新）时，系统会重新向你确认授权。**输入登录密码（开机密码）→ 点「始终允许」**即可，同一版本之后不会再弹。没连接过 GitHub 的用户不会遇到此弹窗。从源码构建的用户：`build.sh` 会用固定的自签名身份签名，重复构建不会反复弹窗（见上文构建说明）。
